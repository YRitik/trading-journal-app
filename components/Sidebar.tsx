...
const navigation = [
  ...
  { name: "Accounts", href: "/accounts", icon: Wallet }, // We updated the icon/link here too
  { name: "Analytics", href: "/analytics", icon: BarChart3 },
];

export default function Sidebar() {
  const pathname = usePathname();
  const { accounts, activeAccountId } = useTrades(); // 2. Get data

  // Find the details of the active account
  ...
  return (
    <div className="flex h-screen w-64 flex-col border-r border-white/5 bg-surface text-text-main">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <item.icon 
                className={`mr-3 h-5 w-5 ${isActive ? "text-primary" : "text-text-muted group-hover:text-text-main"}`} 
              />
              {item.name}
            </Link>
          );
      ...
      {/* User / Active Account Snippet (Bottom) */}
